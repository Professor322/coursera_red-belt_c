//
// Created by professor on 4/11/20.
//
...
public:
	void Book(int64_t time, const string& hotel_name,
		...
		current_time_ = time;
		hotels_[hotel_name].Book({time, client_id, room_count}); //log(Q)
	}
	int ComputeClientCount(const string& hotel_name) {
		return hotels_[hotel_name].ComputeClientCount(current_time_);  //LLog(Q)
	}
